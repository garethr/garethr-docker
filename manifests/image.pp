...
define docker::image(
  ...
  if $docker_dir {
    $image_install = "${docker_command} build ${build_args_string}-t ${image_arg} ${docker_dir}"
  } elsif $docker_file {
    $image_install = "${docker_command} build ${build_args_string}-t ${image_arg} - < ${docker_file}"
  } elsif $docker_tar {
